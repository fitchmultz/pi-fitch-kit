...
Installing the kit is already consent for its bundled resources. Its two prompts and `anthropic-image-guard` load with the package. Installing the pinned `pi-subagents` package supplies the fourteen specialist defaults plus `delegate`; do not create user-level profile copies or symlinks. The image guard applies only to Anthropic requests and matters primarily when global image auto-resizing is disabled. After `/reload`, verify the expected package-owned resources.
