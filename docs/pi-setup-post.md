...
The quality-first diagnosis and GPT review roles use GPT-5.6 Sol. The speed-sensitive scout, context-builder, fixer, and worker use `xai/grok-4.5` at high effort, falling back through `cursor/grok-4.5` before Sol. Fable handles cross-model review and writing. The value still comes from clean context and a clear job, not pretending each agent has a personality.
