...
Use openai-codex/gpt-5.6-sol at max thinking and low verbosity for the main session, with Codex priority mode on. Compact with xai/grok-4.5 at high, then openai-codex/gpt-5.6-luna at high. Route scout, context-builder, fixer, and worker through Pi's built-in xai/grok-4.5 at high effort. All four fall back first to cursor/grok-4.5 and then openai-codex/gpt-5.6-sol; fixer and worker finally fall back to anthropic/claude-opus-5. Route debugger and planner through anthropic/claude-opus-5 at high effort, falling back to anthropic/claude-fable-5 and then openai-codex/gpt-5.6-sol. Authenticate xAI with /login xai using my Grok/X subscription or xAI API key, and save a Cursor SDK API key through /login for the pi-cursor-sdk route.
