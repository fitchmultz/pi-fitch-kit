...
The kit itself bundles stable session naming, the Claude image boundary, shared fast-mode toggles for Anthropic Opus and OpenAI routes, and `/write-prompt`. The profiles ship directly with `pi-subagents`, so there is no second copy or sync layer.
