...
- kit boundary: a compact non-truncating footer, stable session naming, Anthropic-only image resizing, shared fast-mode toggles for Anthropic Opus and OpenAI routes, `/draft`, and `/side-question`, while `pi-subagents` owns its profile defaults.
