...
The public settings example selects `openai/gpt-6-astra` at medium reasoning. My personal main session instead uses `openai-codex/gpt-6-astra` at medium, with a 300k context budget. Updating the kit does not change that choice or replace explicit cross-family reviewer routes.
...
The settings example uses `compaction.reserveTokens: 64000` and `keepRecentTokens: 40000`. The manifest offers 300k context budgets, giving a 236k compaction threshold; my personal budgets use the same 300k. These are selected budgets, not claims that one size or effort level is universally optimal.
