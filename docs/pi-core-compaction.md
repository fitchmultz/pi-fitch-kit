...
2. Read the current request loop, compaction implementation, extension API, changelog, `~/.pi/agent/models.json`, and `~/.pi/agent/settings.json` before editing. Preserve unrelated configuration and provider definitions.
...
2. Run `pi --list-models 'openai/gpt-5.6'` and `pi --list-models 'openai-codex/gpt-5.6'`, then confirm Sol, Terra, and Luna match the active overrides in `models.json` under both providers.
3. Confirm global settings still have compaction enabled with the intended reserve and keep-recent values. Inspect only the structural routing keys in the optional `<Pi agent dir>/pi-codex-context.json`: absent or non-literal consent must keep custom routing off; if `customCompactionEnabled` is `true`, confirm the user approved the exact listed destinations. An omitted model list means xAI Grok 4.5 high before Codex Luna high; a valid override replaces that order.
