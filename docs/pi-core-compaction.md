# Reapply the Pi 0.84.1 compaction, recovery, and diagnostics fixes
...
Summary generation can be routed separately by this package's `extensions/codex-context.ts`, but Pi's active session model is the default. The handler reads `<Pi agent dir>/pi-codex-context.json` at compaction time and returns control to Pi's active-model compaction unless `customCompactionEnabled` is the literal boolean `true` and `compactionModels` is a valid non-empty list. Configured candidates run in listed order before returning control to Pi's active-model compaction if none succeeds. Candidate authentication is raced against the compaction signal so cancellation does not wait for an auth refresh to finish. Missing, invalid, or empty lists fail closed to the active model. The handler must call Pi's native `compact()` with the existing `CompactionPreparation` and dispatch each candidate through Pi 0.84's composed provider stream. OpenAI candidates wrap that stream only to apply the fast-mode payload option. The handler must not replace cut-point selection, retention, cancellation, file tracking, persistence, or compaction ownership.
...
A provider can also be healthy without producing a visible assistant event. Anthropic's Messages stream sends SSE `ping` events, and Pi's adapter intentionally filters them before the assistant event stream. Those bytes correctly reset undici's body-idle timeout, so a long inference can outlive `httpIdleTimeoutMs` while the TUI shows only `Working...`. That diagnosed behavior is accepted. Do not redefine network idle as “no visible model token,” add a watchdog, change provider timeouts, or add Anthropic-specific retries: each would turn healthy slowness into failure.
...
- `dist/modes/interactive/components/status-indicator.js` (tracked only to remove the v0.8.0 legacy hunk; current must match stock)
...
An install patched by kit 0.4.1 through 0.6.0 or by kit 0.8.0 is recognized as a sha-pinned `legacy-patched` state. Kit 0.7.0 is byte-identical to the current runtime artifact and is therefore already `patched`. `apply` migrates each legacy state in one guarded run: it reverses the matching checksum-pinned archive under `patches/archive/`, verifies the exact stock intermediate, ensures the stock backup exists, then applies the current patch; any failure rolls back to the same verified legacy pre-state. When a released backup predates later-tracked core files, `apply` extends it with the missing stock preimages taken from the install (legacy patches never touched them, and anything else fails closed) and atomically rewrites the manifest last. A complete legacy backup needs no new preimages but still refreshes stale recorded patch metadata. `restore` reverses the matching archived patch directly to stock only after confirming the backup records every file that installed era can reverse; a layout valid for a different release is insufficient. Any other divergence still fails closed.
...
1. **Exact generic transient classification.** TARS observed the bare assistant error `Sorry, something went wrong` (with an optional final period). The reachable local path is Codex Responses: `mapCodexEvents` turns `response.failed` into a status-less `CodexApiError` carrying only the provider message, and the Codex catch formats that message without a prefix. Vanilla Responses prefixes its normal `error`, `response.failed`, and HTTP-error paths. The patch adds the deliberately narrow `^Sorry, something went wrong\\.?$` pattern to the shared retry allowlist. Extra text does not match. Retry policy remains Pi's native bounded, abortable backoff; the patch adds no loop or budget. The regression keeps archived v0.6.0 permanently red for this exact shape and current green, while preserving unknown-error fail-fast behavior.
2. **Additive vanilla Responses diagnostics.** `openai-responses.js` records HTTP status and `x-request-id`, including `APIError.requestID` (plus a `request_id` wrapper fallback) when setup or streaming throws. `openai-responses-shared.js` records terminal response status and provider error code in `providerMetadata`, plus response ID in the existing top-level `responseId`, for `error` / `response.failed` events that reach it. The terminal event's ID is preferred and the existing value is the fallback; normal Responses streams carry the same ID in `response.created` and the terminal event. These runtime fields do not change stop-reason mapping or response content; `providerMetadata` is not declared in Pi 0.84.1's emitted `AssistantMessage` type, so typed consumers need the upstream type addition. Vanilla Responses passes those terminal events to the shared processor, so it receives the full diagnostics. Codex uses the shared processor for successful SSE and WebSocket events and therefore gains success-path response status, but `mapCodexEvents` throws on `error` / `response.failed` before the shared failure branches run; its catch adds no metadata. Codex failure diagnostics and HTTP status/request ID therefore remain unchanged, not merely partial. Codex still gains the exact generic retry classification.
...
## Required slow-provider behavior

Keep Pi's stock `WorkingStatusIndicator` unchanged. Healthy long inference may leave the static working row visible until the provider emits an assistant event. Do not emit synthetic assistant content, expose provider heartbeat events, add elapsed-time UI, add a token-silence watchdog, change provider timeouts, or add Anthropic-specific retries.

The focused regression uses local Anthropic-compatible SSE servers. A body with no bytes must still fail at the configured idle timeout. A body sending only filtered `ping` events must outlive that timeout and finish normally while producing no assistant progress until terminal content arrives. A fake-clock check keeps archived v0.8.0 as the red proof for the superseded elapsed indicator and requires current to schedule no disclosure timer, no one-second elapsed interval, and to preserve the stock working message.
...
`<Pi agent dir>/pi-codex-context.json` owns the explicit routing consent and ordered custom summary-model candidates. Missing, malformed, false, non-boolean, or empty configuration must not query or send retained context to alternate models. Routing activates only when `customCompactionEnabled` is literally `true` and `compactionModels` is a valid non-empty list; an omitted, invalid, or empty list falls through to Pi's native active-model compaction. Unauthenticated, unavailable, or failed enabled candidates fall through in order; if all enabled candidates fail, returning no extension result preserves the active-model fallback. When the patched host includes `retry` and `retryCallbacks` on `session_before_compact`, forward both to native `compact()` so a transient failure retries the same candidate before failover and native retry lifecycle events remain intact.
...
3. Using the allowlisted local structural parser from step 2, confirm global settings still have compaction enabled with the intended reserve and keep-recent values. Use the same local allowlist for only the structural routing keys in the optional `<Pi agent dir>/pi-codex-context.json`: absent or non-literal consent, or an omitted, empty, or invalid model list, must keep custom routing off; if custom routing is active, confirm the user approved the exact listed destinations.
4. Run `node --check` on the six modified Pi JavaScript files plus the tracked stock `status-indicator.js`, including `node_modules/@earendil-works/pi-ai/dist/utils/retry.js`, `openai-responses.js`, and `openai-responses-shared.js`.
...
6. Resolve `pi-fitch-kit`'s installed root with `pi list`, then run:
   ...
   Run `npm run regression:anthropic-stall --prefix <package-root>` to prove true byte-idle timeout, healthy filtered-heartbeat survival, the archived v0.8.0 elapsed-indicator red, and the current stock-working-row green.
...
- Custom summary routing makes no alternate-model lookup or request for missing, malformed, false, truthy non-boolean, omitted-list, empty-list, or invalid configuration. Cancelling while alternate-model authentication is pending settles promptly without reaching the provider. An explicit valid list uses native candidates in order, preserves native summary usage, fresh routing session IDs, disabled one-off prompt-cache writes, credential-resolved base URLs, header-only authentication including `null` deletion markers, and OpenAI priority mode, honors cancellation, and returns control to Pi's active-model fallback when every configured candidate fails.
