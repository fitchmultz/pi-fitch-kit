...
- Adopted TARS's reviewed OpenAI Responses resilience patch into the kit's Pi 0.84.1 core artifact, unifying the two patch streams. The shared retry classifier now recognizes only the exact bare transient `Sorry, something went wrong` message (optional final period) and continues to use Pi's native bounded, abortable backoff; extra detail remains non-retryable. Responses output now preserves HTTP status, `x-request-id`, terminal response status/ID, and provider error code in additive `providerMetadata` fields. The shared terminal diagnostics apply to both vanilla and Codex Responses adapters; vanilla-only HTTP metadata means Codex HTTP-level diagnostics remain partial.
