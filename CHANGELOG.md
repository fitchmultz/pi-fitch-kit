...
- Made the exact-version core applicator concurrency- and crash-safe with `/usr/bin/patch`, atomic PID locking, symlink confinement, staged stock backups, action-scoped artifact checksums, canonical manifests, durable recovery journals, early backup-health failures, and targeted wrapper diagnostics. Added isolated security, migration, and interruption fixtures.
