...
- Routed `scout`, `context-builder`, `fixer`, and `worker` through Pi's built-in `xai/grok-4.5` at high effort, with explicit GPT-5.6 Sol fallback, for much faster bounded delegation without giving up the parent's quality gate.
