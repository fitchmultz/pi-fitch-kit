...
- Routed `scout`, `context-builder`, `fixer`, and `worker` through Pi's built-in `xai/grok-4.5` at high effort with explicit GPT-5.6 Sol fallback; consequential research, review gates, and oracle decisions remain xhigh.
...
- Added a version-pinned setup manifest and the preview-first `/fitch-setup` prompt, including exact xAI model and authentication requirements.
