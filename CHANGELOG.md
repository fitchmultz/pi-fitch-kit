...
- Documented the live verification of `/codex-fast` on the `openai-codex` subscription route, which had never been measured: `gpt-5.6-sol` with output pinned at 603 tokens per sample ran 55.5 tok/s and 12502ms standard against 82.6 tok/s and 9074ms fast across three interleaved pairs, a 1.49x throughput and 1.38x completion speedup. No code change: the stock `before_provider_request` payload field already carries it. Recorded two measurement traps found the hard way: the Codex backend echoes `service_tier: "default"` while serving priority (openai/codex#14204), so the echoed tier proves nothing there, and the gain only appears as throughput on a fixed-length output, not as time-to-first-token. Adding the Codex CLI's `x-codex-routing-hint` header measured 84.1 tok/s against 82.6 without it, inside noise, so the kit does not send it and keeps `before_provider_headers` unused.
