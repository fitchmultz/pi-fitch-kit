...
- `npm run smoke` loads the checkout through Pi's real resource loader, renders the compact footer at wide and narrow widths, checks its toggle and in-session model status against legacy-file fixtures, and requires eight commands, `name_session`, `fitch_setup_models`, `reader_present`, one provider request hook, seven extensions, and two prompts.
