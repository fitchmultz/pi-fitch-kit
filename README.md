...
Reader state lives in native session custom entries and follows the active branch through restart, `/tree`, and compaction. On official Pi 0.87.1, native sessions created with only `/reader demo` before their first assistant response still defer the initial JSONL write; restart durability begins once Pi has persisted that session. The maintained fork saves the first custom entry immediately. Pi before 0.85.1 has no native mouse route, so reader controls remain keyboard-only there.
...
```text
extensions/             footer, image guard, fast modes, session naming, paged reader, setup models, writer commands, and /restart
...
- `npm run check` type-checks and syntax-checks the bundled extensions, exercises the image guard, fast toggles, session naming, paged reader, writer commands, and restart boundaries, then validates unpinned package sources, manifest resources, package metadata alignment, the absence of retired patch and duplicate surfaces, and the settings example's model, retry, and compaction consistency. It also runs the validator against invalid manifest and compaction inputs; policy values live in the manifest and settings example rather than a second frozen validator table.
...
- `npm run smoke` loads the checkout through Pi's real resource loader, renders the compact footer at wide and narrow widths, checks its toggle and in-session model status against legacy-file fixtures, and requires eight non-TTY commands, `name_session`, `fitch_setup_models`, `reader_present`, one provider request hook, eight extensions, and two prompts. `/restart` remains inert in this SDK host and is exercised in the real TTY check.
