...
The manifest is the source of truth for package channels, models, bundled resources, and optional service connections. It keeps the released `pi-agent-browser-native` wrapper paired with its tested Agent Browser 0.34.0 baseline. [`examples/settings.json`](examples/settings.json) is a safe subset of my behavioral settings, not a credential-bearing config dump. It mirrors the direct OpenAI Sol default and current ten-route model cycle; setup filters unavailable owner-specific routes when applying `enabledModels`.
