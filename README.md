...
- `npm run regression:fast-mode` verifies the fast toggles at the wire through real pi-ai serialization: `speed` plus fetch-time beta append on direct and gateway Opus routes without dropping existing markers, beta deduplication, prebuilt-client bypass, full-stream option survival, OpenAI and xAI priority payloads, off-state passthrough, footer eligibility including proxy exclusion, and watcher cleanup.
