...
The kit installs public extension packages without patching [Pi](https://github.com/earendil-works/pi). Everything works on stock Pi except the optional compact-view preference, which uses public features in [my Pi fork](https://github.com/fitchmultz/pi/blob/main/FORK.md). Credentials, private provider definitions, and user-local experiments stay user-managed.
...
```text
extensions/             footer, image guard, fast modes, session naming, setup models, and writer commands
...
- `npm run smoke` loads the checkout through Pi's real resource loader, renders the compact footer at wide and narrow widths, checks its toggle and in-session model status against legacy-file fixtures, and requires seven commands, `name_session`, `fitch_setup_models`, one provider request hook, six extensions, and two prompts.
