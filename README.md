...
**Restart requires native activity support.** Use the Pi fork with `ctx.isBashRunning()`, `ctx.getPendingInputCount()` and `ctx.getPendingNextTurnCount()`. They expose intercepted Bash work, submitted input and queued context without relying on extension load order. Hosts without these APIs, including official Pi 0.84.2, report restart as unavailable and refuse both restart modes; they are never treated as idle.
