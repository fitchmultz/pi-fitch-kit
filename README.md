...
The same guarded Pi 0.84.1 core artifact carries bounded transient-error recovery, additive OpenAI Responses diagnostics, and slow-turn visibility. After 30 seconds, the terminal working row shows elapsed time and the interrupt key instead of an indefinitely static `Working...`. This deliberately does not treat Anthropic's filtered SSE heartbeat pings as idle or abort a healthy long inference. Applying any core artifact remains opt-in and requires a full Pi process restart.
...
- `npm run regression:anthropic-stall` proves a real byte-idle Anthropic body still times out, heartbeat pings keep a healthy stream alive without visible assistant progress, v0.7.0 keeps the original silent indicator, and current surfaces elapsed time plus the interrupt key.
