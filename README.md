...
[`fast-mode`](extensions/fast-mode.ts) owns both provider fast toggles in one place. `/anthropic-fast [on|off|toggle|status]` requests Anthropic's research-preview fast mode for Opus 5 and Opus 4.8 at double the token price; it is verified working on this setup's Claude subscription OAuth route, where identical output ran roughly 2x faster with the toggle on. `/codex-fast [on|off|toggle|status]` requests OpenAI's priority service tier on the `openai` and `openai-codex` providers. Both toggles ride Pi's stock `before_provider_request` and `before_provider_headers` hooks, so the kit overrides no provider streams and `/reload` is always enough. Anthropic eligibility follows the wire API rather than the provider name, so Claude Opus through `cloudflare-ai-gateway` gets the same toggle as the direct Anthropic route, and models that ignore fast mode never receive the `speed` field or its mandatory beta header. Toggle state lives in the shared per-user `anthropic-fast.json` and `openai-codex-fast.json` files; the footer shows `fast:on` or `fast:off` only while an eligible model is selected and follows changes made in other sessions. Two accepted tradeoffs: reported cost rates are not doubled while Anthropic fast mode is on, and Pi-internal summarization requests such as compaction stay at standard speed.
...
- `npm run regression:fast-mode` verifies both toggles: `speed`/beta-header injection on eligible Anthropic routes (direct and gateway), OpenAI priority payloads, off-state and ineligible-model passthrough, beta deduplication, footer eligibility, and watcher cleanup.
...
- `npm run smoke` loads the checkout through Pi's real resource loader, renders the compact footer at wide and narrow widths, checks its toggle, and requires the three bundled commands, `name_session`, one provider request hook, one provider headers hook, four extensions, and two prompts.
