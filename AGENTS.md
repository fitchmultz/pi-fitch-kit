...
- Use configured agent defaults first: worker/researcher/scout/context-builder are medium-effort, reviewer/fixer are high-effort, ui-designer is xhigh, and oracle is xhigh forked-context by default. Override worker/researcher upward only when the child owns high-risk, hard-debug, broad multi-file, architecture/API/security, data-loss, lifecycle/state, release-blocking, or expensive-to-repeat work.
