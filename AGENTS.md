...
- Keep `extensions/fast-mode.ts` scoped: OpenAI and xAI priority ride `before_provider_request` for `openai`/`openai-codex`/`xai` plus `cloudflare-ai-gateway` models whose id starts with `gpt-` (OpenAI toggle) or `grok-` (xAI toggle) only; Anthropic fast mode owns the `anthropic-messages` override for only the `anthropic` and `cloudflare-ai-gateway` providers, appends the beta at fetch time, and never sends `speed` when the header cannot be attached. Keep the gateway endpoint-placeholder resolution in `fastStream`, the existing state filenames, and the doubled Anthropic fast cost rates.
